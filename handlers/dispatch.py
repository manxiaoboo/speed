from handlers import idle_handler, line1_handler, line2_handler, line3_handler, line4_handler, line5_handler, findABC_hander
import time
import cv2
import enums
import local_status
...
def next():
        ...
        if local_status.isIDLE():
            ...
            isDone = findABC_hander.onImageReceived(frame)
            if isDone:
                setStatus(enums.Status.FindABC)
        elif local_status.isFind123():    
            line1_handler.onImageReceived(frame)
        else:
            time.sleep(1)
